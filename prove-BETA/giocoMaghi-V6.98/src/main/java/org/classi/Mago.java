...
public class Mago extends Thread {
    ...
    private final Vector<Mago> codaTurni;
    ...
    public Mago(String nome, int max, int min, Vector<Mago> codaTurni, AtomicBoolean duelloInCorso, int squadra, int livelloVisioneLog) {
        this.nome = nome;
        ...
        try {
            while (duelloInCorso.get() && vita > 0 && !Thread.currentThread().isInterrupted()) {
                ...
                synchronized (codaTurni) {
                    ...
                    codaTurni.add(this);
                    while (!codaTurni.isEmpty() && codaTurni.firstElement() != this && duelloInCorso.get() && !Thread.currentThread().isInterrupted()) {
                        codaTurni.wait();
                    }
                    if (!duelloInCorso.get() || Thread.currentThread().isInterrupted()) break;
                }
                ...
                if (nemico == null || nemico.getVita() <= 0 || nemico.equals(this) ||
                    ...
                    if (nemico == null) {
                        ...
                        synchronized (codaTurni) {
                            if (!codaTurni.isEmpty()) codaTurni.remove(0);
                            codaTurni.notifyAll();
                        }
                        continue;
                    }
