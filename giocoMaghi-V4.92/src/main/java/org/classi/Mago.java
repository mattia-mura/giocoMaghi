...
public class Mago extends Thread {
    ...
    public Mago(String nome, int max, int min, Queue<Mago> codaTurni, AtomicBoolean duelloInCorso, int squadra) {
        this.nome = nome;
    ...
    private synchronized int calcolaAttacco() {
        ...
        if (!ignoraResistenza) {
            ...
            if (attacco < resistenzaNemico) {
                if (attacco < resistenzaNemico + percentualeResistenza) {
    ...
    public void run() {
        try {
            ...
            if (vita > 0 && !Thread.currentThread().isInterrupted()) {
    ...
    private synchronized void subisciDanno(int danno) {
        ...
        if (this.vita <= 0) {
            ...
            } else if (SquadMaghi.vettoreDiVettori.size() == 1 && this.squadra != -1) {
                // Se rimane solo una squadra, quella squadra vince
                System.out.println("La squadra " + (this.squadra + 1) + " ha vinto il Ragnarok!");
                Trascrittore.scrivi("La squadra " + (this.squadra + 1) + " ha vinto il Ragnarok!");
                duelloInCorso.set(false);
                synchronized (codaTurni) {
                    codaTurni.clear();
                    codaTurni.notifyAll();
    ...
    public synchronized void assegnaNuovoNemico2() {
        ...
        for (Mago m : ListaMaghi.getMaghi()) {
            if (m != this && m.getVita() > 0) {
                // Se entrambi hanno squadre diverse (non -1), possono combattere
                if (this.getSquadra() != -1 && m.getSquadra() != -1) {
                    if (m.getSquadra() != this.getSquadra()) {
                        potenzialiNemici.add(m);
                    }
                } else {
                    // Se almeno uno non ha squadra (-1), possono combattere
                    potenzialiNemici.add(m);
                }
            }
        }
