    ...
    public void run() {
        try {
            while (duelloInCorso.get() && vita > 0 && !Thread.currentThread().isInterrupted()) {
                ...
                if (nemico != null) {
                    if (nemico.getNemico() != null) {
                        if (this.livelloVisioneLog<=2){System.out.println(coloreSquadra + nome + COLORI[0] + " infligge " + coloreElemento + danno + COLORI[0] +
                                " a " + nemico.getNemico().getColoreSquadra() + nemico.getNemico().getNome() + COLORI[0]);}
                    } else {
                        if (this.livelloVisioneLog<=2){System.out.println(coloreSquadra + nome + COLORI[0] + " infligge " + coloreElemento + danno + COLORI[0] +
    ...
    private synchronized void subisciDanno(int danno) {
        if (abilita.equals("Corpo d'acciaio")) {
            ...
            if (chance <= percentuale) {
                ...
                if(this.livelloVisioneLog == 1){System.out.println(coloreSquadra + nome + "\u001B[0m attiva 'Corpo d'acciaio': danno ridotto!");}
        ...
        if (this.vita <= 0) {
            System.out.println(coloreSquadra + nome + "\u001B[0m è stato sconfitto!");
            Trascrittore.scrivi(coloreSquadra + nome + "\u001B[0m è stato sconfitto!");

            // Rimuovi il mago morto
            ListaMaghi.removeMago(this);
            SquadMaghi.rimuoviMago(this);

            // Aggiorna i nemici degli altri maghi
            for (Mago m : ListaMaghi.getMaghi()) {
                if (m.getNemico() == this) {
                    m.assegnaNuovoNemico2();
                    powerUP(m);
                    if (this.livelloVisioneLog<=2){System.out.println(coloreSquadra+m.getNome() + "\u001B[0m" + " ha ricevuto un powerUP!");}
...
//            System.out.println("CONDIZIONE 2: Controllo squadra vincente...");
            ...
            if (tuttaStessaSquadra && squadraComune != -1) {
                System.out.println(this.coloreSquadra + " Tutta squadra " + squadraComune + " ha vinto il Ragnarok!" + "\u001B[0m");
                Trascrittore.scrivi("Tutta squadra " + squadraComune + " ha vinto il Ragnarok!");
                duelloInCorso.set(false);
                synchronized (codaTurni) {
                    codaTurni.clear();
                    codaTurni.notifyAll();
    ...
    public synchronized void assegnaNuovoNemico2() {
        ...
        if (this.livelloVisioneLog == 1){System.out.println(coloreSquadra + nome + "\u001B[0m" + " ha scelto " + nemico.getColoreSquadra() + nemico.getNome() + "\u001B[0m" + " come nuovo nemico!");}
        // La stampa già colorata del nome del nemico
    }
    ...
    public int getSquadra() { return squadra; }
    ...
    public String getColoreSquadra(){return this.coloreSquadra;}
    @Override
